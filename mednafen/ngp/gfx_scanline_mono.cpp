...
static void MonoPlot(uint8 x, uint8* palette_ptr, uint16 pal_hi, uint8 index, uint8 depth)
{
   uint8 data8;
   ...
   if (gfx->negative)
   	  gfx->cfb_scanline[x] = (r | g | b);
   else
   	  gfx->cfb_scanline[x] = ~(r | g | b);
}

static void drawMonoPattern(uint8 screenx, uint16 tile, uint8 tiley, uint16 mirror, 
                            uint8* palette_ptr, uint16 pal, uint8 depth)
{
   // Get the data for th e "tiley'th" line of "tile".
   uint16 data = MDFN_de16lsb(gfx->CharacterRAM + (tile * 16) + (tiley * 2));
...
static void draw_mono_scroll1(uint8 depth, int ngpc_scanline)
{
   uint8 tx, row, line;
   uint16 data16;
   ...
   for (tx = 0; tx < 32; tx++)
   {
      ...
      drawMonoPattern((tx << 3) - gfx->scroll1x, data16 & 0x01FF, 
                      (data16 & 0x4000) ? 7 - row : row, data16 & 0x8000, gfx->SCRP1PLT,
			          data16 & 0x2000, depth);
   }
}

static void draw_mono_scroll2(uint8 depth, int ngpc_scanline)
{
   uint8 tx, row, line;
   uint16 data16;
   ...
   for (tx = 0; tx < 32; tx++)
   {
      ...
      //Draw the line of the tile
      drawMonoPattern((tx << 3) - gfx->scroll2x, data16 & 0x01FF, 
                      (data16 & 0x4000) ? 7 - row : row, data16 & 0x8000, gfx->SCRP2PLT,
			          data16 & 0x2000, depth);
   }
}

void NGPGfx_draw_scanline_mono(int layer_enable, int ngpc_scanline)
{
   int16 lastSpriteX;
   int16 lastSpriteY;
   int spr;
   ...
   //Window colour
   uint16 r = (uint16)gfx->oowc << 1;
   uint16 g = (uint16)gfx->oowc << 5;
   uint16 b = (uint16)gfx->oowc << 9;
