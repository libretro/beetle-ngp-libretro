//---------------------------------------------------------------------------
// NEOPOP : Emulator as in Dreamland
//
// Copyright (c) 2001-2002 by neopop_uk
//---------------------------------------------------------------------------

//---------------------------------------------------------------------------
//	This program is free software; you can redistribute it and/or modify
//	it under the terms of the GNU General Public License as published by
//	the Free Software Foundation; either version 2 of the License, or
//	(at your option) any later version. See also the license.txt file for
//	additional informations.
//---------------------------------------------------------------------------

#include "neopop.h"
#include "mem.h"
#include "gfx.h"

//=============================================================================

#define gfx NGPGfx

static const unsigned char mirrored[] = {
    
};

//=============================================================================


static void drawColourPattern(uint8 screenx, uint16 tile, uint8 tiley, uint16 mirror, 
                              uint16* palette_ptr, uint8 pal, uint8 depth)
{
   int index, x, left, right, highmark, xx;
   uint16 data16;

   x = screenx;
   if (x > 0xf8)
      x -= 256;
   if (x >= SCREEN_WIDTH)
      return;

   // Get the data for the "tiley'th" line of "tile".
   index = MDFN_de16lsb(gfx->CharacterRAM + (tile * 16) + (tiley * 2));

   // Horizontal Flip
   if (mirror)
      index = mirrored[(index & 0xff00)>>8] | (mirrored[(index & 0xff)] << 8);

   palette_ptr += pal << 2;
   left = MAX(MAX(x, gfx->winx), 0);
   right = x+7;

   highmark = MIN(gfx->winw + gfx->winx, SCREEN_WIDTH)-1;

   if (right > highmark) {
      index >>= (right - highmark)*2;
      right = highmark;
   }

   for (xx = right; xx>=left; --xx,index>>=2) {
      if (depth <= gfx->zbuffer[xx] || (index&3)==0) 
         continue;
      gfx->zbuffer[xx] = depth;

      // Get the colour of the pixel
      data16 = MDFN_de16lsb(&palette_ptr[index&3]);

      if (gfx->negative)
         gfx->cfb_scanline[xx] = ~data16;
      else
         gfx->cfb_scanline[xx] = data16;
   }
}

static void draw_colour_scroll1(uint8 depth, int ngpc_scanline)
{
   uint8 tx, row, line;
   uint16 data16;

   line = ngpc_scanline + gfx->scroll1y;
   row = line & 7; // Which row?

   // Draw Foreground scroll plane (Scroll 1)
   for (tx = 0; tx < 32; tx++)
   {
      data16 = MDFN_de16lsb(gfx->ScrollVRAM + ((tx + ((line >> 3) << 5)) << 1));
		
      // Draw the line of the tile
      drawColourPattern((tx << 3) - gfx->scroll1x, data16 & 0x01FF, 
			            (data16 & 0x4000) ? (7 - row) : row, data16 & 0x8000, (uint16*)(gfx->ColorPaletteRAM + 0x0080),
			            (data16 & 0x1E00) >> 9, depth);
   }
}

static void draw_colour_scroll2(uint8 depth, int ngpc_scanline)
{
   uint8 tx, row, line;
   uint16 data16;

   line = ngpc_scanline + gfx->scroll2y;
   row = line & 7; // Which row?

   // Draw Background scroll plane (Scroll 2)
   for (tx = 0; tx < 32; tx++)
   {
      data16 = MDFN_de16lsb(gfx->ScrollVRAM + 0x0800 + ((tx + ((line >> 3) << 5)) << 1));
		
      // Draw the line of the tile
      drawColourPattern((tx << 3) - gfx->scroll2x, data16 & 0x01FF, 
			            (data16 & 0x4000) ? (7 - row) : row, data16 & 0x8000, (uint16*)(gfx->ColorPaletteRAM + 0x0100),
			            (data16 & 0x1E00) >> 9, depth);
   }
}

void NGPGfx_draw_scanline_colour(int layer_enable, int ngpc_scanline)
{
   int16 lastSpriteX;
   int16 lastSpriteY;
   int spr;
   uint16 win_color;

   memset(gfx->cfb_scanline, 0, SCREEN_WIDTH * sizeof(uint16));
   memset(gfx->zbuffer, 0, SCREEN_WIDTH);

   //Window colour
   win_color = MDFN_de16lsb(gfx->ColorPaletteRAM + 0x01F0 + (gfx->oowc << 1));
   if (gfx->negative) win_color = ~win_color;

   //Top
   if (ngpc_scanline < gfx->winy)
   {
      for (int x = 0; x < SCREEN_WIDTH; x++)
         gfx->cfb_scanline[x] = win_color;
   }
   else
   {
      //Middle
      if (ngpc_scanline < gfx->winy + gfx->winh)
      {
		 for (int x = 0; x < MIN(gfx->winx, SCREEN_WIDTH); x++)
            gfx->cfb_scanline[x] = win_color;

         for (int x = MIN(gfx->winx + gfx->winw, SCREEN_WIDTH); x < SCREEN_WIDTH; x++)
            gfx->cfb_scanline[x] = win_color;
      }
      else //Bottom
      {
         for (int x = 0; x < SCREEN_WIDTH; x++)
            gfx->cfb_scanline[x] = win_color;
      }
   }

   //Ignore above and below the window's top and bottom
   if (ngpc_scanline >= gfx->winy && ngpc_scanline < gfx->winy + gfx->winh)
   {
      //Background colour Enabled?	HACK: 01 AUG 2002 - Always on!
      // if ((gfx->bgc & 0xC0) == 0x80)
      {
         win_color = MDFN_de16lsb(gfx->ColorPaletteRAM + 0x01E0 + ((gfx->bgc & 7) << 1));
      }
      // else win_color = 0;

      if (gfx->negative) win_color = ~win_color;

      //Draw background!
      for (int x = gfx->winx; x < MIN(gfx->winx + gfx->winw, SCREEN_WIDTH); x++)	
         gfx->cfb_scanline[x] = win_color;

      //Swap Front/Back scroll planes?
      if (gfx->planeSwap)
      {
         if(layer_enable & 1)
            draw_colour_scroll1(ZDEPTH_BACKGROUND_SCROLL, ngpc_scanline); //Swap
         if(layer_enable & 2)
            draw_colour_scroll2(ZDEPTH_FOREGROUND_SCROLL, ngpc_scanline);
      }
      else
      {
         if(layer_enable & 1)
            draw_colour_scroll2(ZDEPTH_BACKGROUND_SCROLL, ngpc_scanline); //Normal
         if(layer_enable & 2)
            draw_colour_scroll1(ZDEPTH_FOREGROUND_SCROLL, ngpc_scanline);
      }

      //Draw Sprites
      //Last sprite position, (defaults to top-left, sure?)
      lastSpriteX = 0;
      lastSpriteY = 0;

      if(layer_enable & 4)
      for (spr = 0; spr < 64; spr++)
      {
         uint8 priority, row;
         uint8 sx = gfx->SpriteVRAM[(spr * 4) + 2];	//X position
         uint8 sy = gfx->SpriteVRAM[(spr * 4) + 3];	//Y position
         int16 x = sx;
         int16 y = sy;
         uint16 data16;

         data16 = MDFN_de16lsb(gfx->SpriteVRAM + (spr * 4));
         priority = (data16 & 0x1800) >> 11;

         if (data16 & 0x0400) x = lastSpriteX + sx;	//Horizontal chain?
         if (data16 & 0x0200) y = lastSpriteY + sy;	//Vertical chain?

         //Store the position for chaining
         lastSpriteX = x;
         lastSpriteY = y;

         //Visible?
         if (priority == 0)	continue;

         //Scroll the sprite
         x += gfx->scrollsprx;
         y += gfx->scrollspry;

         //Off-screen?
         if (x > 248 && x < 256) x = x - 256; else x &= 0xFF;
         if (y > 248 && y < 256) y = y - 256; else y &= 0xFF;

         //In range?
         if (ngpc_scanline >= y && ngpc_scanline <= y + 7)
         {
            row = (ngpc_scanline - y) & 7; //Which row?
            drawColourPattern((uint8)x, data16 & 0x01FF, 
                              (data16 & 0x4000) ? 7 - row : row, data16 & 0x8000,
                              (uint16*)gfx->ColorPaletteRAM, gfx->SpriteVRAMColor[spr] & 0xF, priority << 1); 
         }
      }

      //==========
   }

}

//=============================================================================

#undef gfx
