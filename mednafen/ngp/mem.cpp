...
uint8 loadB(uint32 address)
{
   ...
   ptr = (uint8*)translate_address_read(address);

   if (ptr)
      return *ptr;

   if(address >= 0x8000 && address <= 0xbfff)
      return(NGPGfx_read8(address));
...
uint16 loadW(uint32 address)
{
   ...
   if(address >= 0x8000 && address <= 0xbfff)
      return(NGPGfx_read16(address));

   if(address >= 0x4000 && address <= 0x7fff)
   {
      return MDFN_de16lsb(&CPUExRAM[(size_t)address - 0x4000]);
   }
   if(address == 0x50)
      return(SC0BUF);
...
void storeW(uint32 address, uint16 data)
{
   ...
   if(address >= 0x8000 && address <= 0xbfff)
   {
      NGPGfx_write16(address, data);
      return;
   }
   if(address >= 0x4000 && address <= 0x7fff)
   {
      MDFN_en16lsb(&CPUExRAM[(size_t)address - 0x4000], data);
      return;
   }
   if(address >= 0x70 && address <= 0x7F)
   {
   ...
   ptr = (uint16*)translate_address_write(address);
